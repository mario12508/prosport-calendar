...
class Home(ListView):
    ...
    def get_queryset(self):
        ...
        if event_period == 'upcoming':
            ...
            queryset = queryset.filter(date_start__gte=now, date_end__lte=end_date)
        elif event_period == 'this_week':
            # Фильтрация мероприятий текущей недели
            start_of_week = now - timedelta(days=now.weekday())
            end_of_week = start_of_week + timedelta(days=6)

            queryset = queryset.filter(date_start__gte=start_of_week,
                                       date_start__lte=end_of_week)
            ...
            queryset = queryset.filter(date_start__gte=next_half_year_start,
            ...
            queryset = queryset.filter(date_start__gte=timezone.now(),
                                       date_start__lte=next_quarter)
        elif event_period == 'next_half_year':
            # Фильтрация мероприятий полугодия
            next_half_year_start = now
            next_half_year_end = now + timedelta(weeks=26)
            queryset = queryset.filter(date_start__gte=next_half_year_start, date_start__lte=next_half_year_end)
        ...
        if gender:
            if gender == "Муж.":
                queryset = queryset.filter(
                    text__icontains="юниоры") | queryset.filter(
                    text__icontains="мужчины") | queryset.filter(
                    text__icontains="юноши") | queryset.filter(
                    text__icontains="мальчики")
            else:
                queryset = queryset.filter(
                    text__icontains="женщины") | queryset.filter(
                    text__icontains="юниорки") | queryset.filter(
                    text__icontains="девушки") | queryset.filter(
                    text__icontains="девочки")
        if group:
